...
int fire(struct Car* head) {
    head = head->next;
    int totalPeople = head->passenger, train = 0;
    while(totalPeople < head->flammable && head->next != NULL){
        head = head->next;
        train++;
        totalPeople += head->passenger;
        //printf("%d %d %d\n",train, totalPeople, head->flammable);
    }
    if(head->next == NULL && totalPeople <= head->flammable) train++;
    return train;
}
