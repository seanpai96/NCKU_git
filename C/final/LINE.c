...
int cmp(const void* a, const void* b) {
    struct message_info *eA = (struct message_info *)a;
    struct message_info *eB = (struct message_info *)b;
    
    int timeA = eA->time.hour*100+eA->time.minute;
    int timeB = eB->time.hour*100+eB->time.minute;
